...
# rows_list = [8, 0, 4, 3, 3, 12, 7, 2, 8, 10]
# columns_list_first = [2, 0, 1, 1, 1, 2, 2, 1, 2, 2]
# columns_list_second = [82, 0, 777, 777, 777, 202, 400, 777, 252, 152]
...
# -------------------------------- РАСЧЕТНЫЙ ЦИКЛ ----------------------------------------
    ...
    for i in range(years_len):
        ...
        prices_yearly_signal = np.where(prices_yearly_returns[str(int(cheked_year) + i)].iloc[-1] > 0, 0, 1)
        ...
        Data_for_Portfolio_master_filter['Total Score'] = (Data_for_Portfolio_master_filter['Valuation Score'] * \
                    Valuation_weights) + (Data_for_Portfolio_master_filter['Quality Score'] * Quality_weights) + \
                    (Data_for_Portfolio_master_filter['Grows score'] * Growth_weights ) + \
                    (Data_for_Portfolio_master_filter['Momentum Score'] * MOM_weights) + \
                    (Data_for_Portfolio_master_filter['Trend Score'] * Trend_weights) + \
                    (Data_for_Portfolio_master_filter['Shareholder Yield Score'] * Div_weights)
        ...
        Data_for_Portfolio_master_filter.to_excel('Total Score NEGATIVE.xlsx')
...
#
# Returnez_finish[::-1].to_excel('Trend(swings).xlsx')
