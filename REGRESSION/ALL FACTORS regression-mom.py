...
def swings(df, high, low, argrel_window):
    ...
    while ((high_low[swing_high_low].shift(1) * high_low[swing_high_low] > 0)).any():
        ...
        high_low.loc[(high_low[swing_high_low].shift(1) * high_low[swing_high_low] < 0) &
                     (high_low[swing_high_low].shift(1) < 0) & (np.abs(high_low[swing_high_low].shift(1)) < high_low[
            swing_high_low]), swing_high_low] = np.nan
...
for list, index, exchange, exchange_yahoo, row, columns_first, columns_second in zip(LIST_list, index_list, \
                                                                                     exchange_list, exchange_yahoo_list,
                                                                                     rows_list, columns_list_first,
                                                                                     columns_list_second):
            ...
            try:
                ...
                for year_div_5 in range(len(Data_for_Portfolio)):
                    ...
                    if len(Data_for_Portfolio) > len(div_yield_list_5y):
                        ...
                        dividend_payout_ratio_list_5y.append(
                            np.mean(Data_for_Portfolio['Dividend Payout Ratio'][year_div_5:5 + year_div_5]))
                        # num +=1
                    else:
                        pass
...
Returnez_finish[::-1].to_excel('ReturnS_val_mom.xlsx')
